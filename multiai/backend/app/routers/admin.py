...
if not ADMIN_TOKEN:
    raise RuntimeError("SECURITY: ADMIN_TOKEN env var is empty. Refusing to boot.")
...
# ── PHASE-2: FX Rates Management ─────────────────────────────────────────────

@admin.get("/fx")
async def get_current_fx(db: AsyncSession = Depends(get_session)):
    """Get current FX rate."""
...
async def fx_history(days: int = 30, db: AsyncSession = Depends(get_session)):
    """Get FX rate history."""
    rows = await db.execute(
...
@admin.post("/fx")
async def set_fx_rate(fx: FxRateSet, db: AsyncSession = Depends(get_session)):
    """Set today's FX rate. Checks circuit breaker first."""
    # Check if rate spiked > 5% from last pricing rate
...
@admin.get("/users")
async def list_users(limit: int = 50, db: AsyncSession = Depends(get_session),
...
async def user_topup(t: UserTopup, db: AsyncSession = Depends(get_session)):
    """Admin topup for a user."""
    ws = WalletService(db)
    try:
        new_balance = await ws.topup(t.user_id, t.amount_irr,
                                     note=t.note or f"admin topup by admin")
...
async def user_update(u: UserUpdatePlan, db: AsyncSession = Depends(get_session)):
...
async def user_ledger(user_id: int, limit: int = 50,
...
# ── PHASE-3: Brakes & System ─────────────────────────────────────────────────

@admin.get("/brakes/status")
async def brakes_status(db: AsyncSession = Depends(get_session)):
...
async def run_margin_brake(db: AsyncSession = Depends(get_session)):
...
async def register_telegram_user(t: TelegramRegister,
                                  db: AsyncSession = Depends(get_session)):
    ...
    # Create wallet with test credit
    wallet = Wallet(user_id=user.id, balance_irr=100_000)  # 100K IRR test credit
    db.add(wallet)
    await db.commit()
