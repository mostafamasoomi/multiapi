...
class WalletService:
    ...
    async def get_or_create_user(self, user_id: int) -> User:
        u = await self.db.scalar(select(User).where(User.id == user_id))
        if not u:
            u = User(id=user_id, status="active")
            self.db.add(u)
    ...
    async def place_hold(self, user_id: int, estimated_cost_irr: int,
        ...
        # ATOMIC: row-lock + conditional decrement (never negative, no race)
        res = await self.db.execute(
            text("UPDATE wallets SET balance_irr = balance_irr - :amt, "
                 "updated_at = now() WHERE user_id = :uid AND balance_irr >= :amt "
                 "RETURNING id"),
            {"amt": required, "uid": user_id})
        row = res.first()
        if not row:
            w = await self.get_or_create_wallet(user_id)
